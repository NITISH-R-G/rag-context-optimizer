...
def get_repo_context() -> dict[str, Any]:
    ...
    try:
        ...
        logging.getLogger(__name__).warning(f"Could not load knowledge graph: {e}")
        context["graph"] = {}
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            f"Could not load architecture diagrams: {e}"
        )
    ...
    try:
        ...
        logging.getLogger(__name__).warning(f"Could not load README: {e}")
        context["readme"] = ""
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a senior technical writer and software architect. Return ONLY valid JSON matching the requested keys.",
                },
...
def main():
    ...
    if not api_key:
        logging.getLogger(__name__).warning(
            "No OPENAI_API_KEY found. AI Documentation generation skipped."
        )

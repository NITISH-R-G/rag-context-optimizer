...
logging.basicConfig(level=logger.INFO, format='%(levelname)s: %(message)s')
...
def post_pr_comment(markdown_report):
    ...
    if pr_number:
        ...
        try:
            ...
            subprocess.run(["gh", "pr", "comment", pr_number, "-F", "temp_report.md"], check=True, shell=False)  # nosec
...
def main():
    ...
    if not api_key:
        logger.warning("No OPENAI_API_KEY found. Generating a basic AI insights template.")

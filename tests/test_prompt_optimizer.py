...
def test_fit_citations_into_prompt():
    ...
    citations = ["doc1", "doc2"]
    result, _, _ = _fit_citations_into_prompt(prompt, citations, 10, 5, prompt, "balanced")
...
def test_rewrite_prompt_fallback():
    result, _, _ = _rewrite_prompt_fallback(
...
def test_fit_citations_into_prompt_aggressive():
    ...
    citations = ["doc1", "doc2"]
    result, _, _ = _fit_citations_into_prompt(prompt, citations, 10, 5, prompt, "aggressive")
    assert isinstance(result, str)

def test_fit_citations_into_prompt_grounded():
    prompt = "This is a prompt."
    citations = ["doc1", "doc2"]
    result, _, _ = _fit_citations_into_prompt(prompt, citations, 10, 5, prompt, "grounded")

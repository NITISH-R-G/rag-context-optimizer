...
class MockRetriever:
    def hybrid_score(self, _query, _chunk):
        return 0.5

    def bm25_score(self, _query, _chunk):
        return 0.5

    def cross_encoder_score(self, _query, _chunk):
        return 0.5

    def embedding_similarity(self, _q1, _q2):
        return 0.5

    def keyword_overlap_score(self, _query, _chunk):
        return 0.5
...
def test_optimize_with_torch_with_chunks(monkeypatch):
    ...
    demos = [
        DemoCase(
            name="demo1",
            query="test query",
            positive_chunk_ids=("chunk1",),
            expected_citations=("chunk1",),
            preferred_domains=("test",),
        )
    ]
    ...
    assert (
        result == [0.1] * 10
    )  # Since our mock tolist returns [0.1] * len(10 for init)

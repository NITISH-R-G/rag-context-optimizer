...
def test_fallback_plan():
    assert "Verify outage evidence" in _fallback_plan(
        {"task_name": "refund_triage_easy"}
    )
    assert "Align the incident timeline" in _fallback_plan(
        {"task_name": "cross_function_brief_medium"}
    )
...
def test_fallback_action_inspect_limit():
    observation = {
        ...
        "prioritized_artifacts": [
            "chunk2",
            "chunk3",
        ],  # Make both prioritized so prioritize branch is skipped
        "available_artifacts": [
            {"chunk_id": "chunk1", "tokens": 20, "keywords": ["test"]},
            {"chunk_id": "chunk2", "tokens": 20, "keywords": ["test"]},
...
@patch("inference._model_name", return_value="test")
@patch("inference._resolve_llm_credentials", return_value=(None, None, None))
@patch("inference.ALLOW_BASELINE_FALLBACK", False)
def test_run_task_http_no_fallback(mock_creds, mock_name):
    import asyncio
    from inference import _run_task_http

    score, rewards, steps, success = asyncio.run(_run_task_http("refund_triage_easy"))
    assert not success
    assert steps == 0

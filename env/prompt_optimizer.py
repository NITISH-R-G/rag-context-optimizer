...
def _rank_and_select_chunks(
    env: RagContextOptimizerEnv, tuning: Any, clean_prompt: str, mode: CompressionMode
) -> None:
    ranked_candidates = []
    for chunk in env.available_chunks:
...
async def optimize_prompt(
    ...
    tuning = env.last_tuning or env.context_tuner.tune(
        clean_prompt, env.available_chunks
    )
    ...
    if llm_configured():
        ...
        optimized_prompt, citation_ready, citation_guidance = _rewrite_prompt_fallback(
            clean_prompt,
            input_tokens,
            target_tokens,
            mode,

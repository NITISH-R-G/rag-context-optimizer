...
async def optimize_prompt(
    ...
    if not preserve_short_prompt:
        for chunk_id in env._selected_chunks:
            ...
            if chunk is None:
                continue
            best = _summarize_chunk_for_output(
                chunk, env._effective_chunk_text(chunk_id)
            )
    ...
    if llm_configured():
        ...
        short_prompt_rewrite = (
            _lightweight_short_prompt_rewrite(clean_prompt)
        ...
        optimized_prompt, citation_ready, citation_guidance = (

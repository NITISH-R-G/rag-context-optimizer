...
class RagContextOptimizerEnv:
    ...
    def __init__(
        ...
        self._corpus_family = (
            corpus_family_override or os.getenv("RAG_CORPUS_FAMILY") or "enterprise_v1"
        )
        explicit_path = os.getenv("RAG_CORPUS_PATH")
        self._corpus_path = resolve_corpus_path(
            explicit_path, family=None if explicit_path else self._corpus_family
        )
        self._all_chunks = load_corpus(self._corpus_path)
        self._query_overridden = bool(query_override and query_override.strip())
        self._include_project_chunks = os.getenv(
            "ENABLE_PROJECT_CORPUS", ""
        ).strip().lower() in {"1", "true", "yes"}
        self._project_chunks = (
            self._load_project_chunks() if self._include_project_chunks else []
        )
        ...
        self.available_chunks: list[Chunk] = []
    ...
    def _build_task(
        ...
        if query_override and query_override.strip():
            updated_task = replace(
                updated_task, query=query_override.strip(), domain_filter=None
            )
    ...
    async def reset(self) -> StepResult:
        ...
        self.available_chunks = self._rank_chunks_for_query(
            self.task.query, candidate_chunks
        )
        if not self._query_overridden:
            chunk_by_id = {chunk.chunk_id: chunk for chunk in candidate_chunks}
            existing_ids = {c.chunk_id for c in self.available_chunks}
            for chunk_id in (
                self.task.required_artifact_ids + self.task.optional_artifact_ids
            ):
    ...
    async def step(self, action: RagAction) -> StepResult:
        ...
        if action.action_type == "inspect_artifact":
            ...
            reward, info = self._handle_compress(
                artifact_id, float(action.compression_ratio or 0.0)
            )
    ...
    async def state(self) -> dict:
            ...
            prioritized_artifact_details.append(
                {
                    ...
                    "compression_ratio": round(
                        self._compression_ratios.get(chunk_id, 1.0), 3
                    ),
        ...
        optimized_prompt_tokens = (
            await estimate_tokens(optimized_prompt) if optimized_prompt else 0
        )
        return {
            ...
            "available_artifact_ids": [
                chunk.chunk_id for chunk in self.available_chunks
            ],
            "available_chunk_ids": [chunk.chunk_id for chunk in self.available_chunks],
            ...
            "context_tuning": (
                {
                    "mode": self.last_tuning.mode,
                    "top_demo_cases": self.last_tuning.top_demo_cases,
                    "suggested_citations": self.last_tuning.suggested_citations,
                    "token_dropout": self.last_tuning.token_dropout,
                    "leave_one_out": self.last_tuning.leave_one_out,
                }
    ...
    def _rank_chunks_for_query(
        self, query: str, chunks: list[Chunk], top_k: int = 20
    ) -> list[Chunk]:
        ...
        for chunk in chunks:
            ...
            score = (
                ...
                else self.retriever.hybrid_score(query, chunk)
            )
            if (
                self._include_project_chunks
                and self._query_overridden
                and chunk.domain.startswith("Project")
            ):
    ...
    def _load_project_chunks(self) -> list[Chunk]:
        ...
        file_specs = [
            ...
            (
                ...
                root / "openenv.yaml",
                ["project_docs", "config", "openenv_spec"],
            ),
            ("Project API", root / "app.py", ["project_docs", "api", "server"]),
            (
                "Project Baseline",
                root / "inference.py",
                ["project_docs", "baseline", "inference"],
            ),
            (
                "Project Environment",
                root / "env" / "environment.py",
                ["project_docs", "environment", "state_management"],
            ),
            (
                "Project Retrieval",
                root / "env" / "retriever.py",
                ["project_docs", "retrieval", "ranking"],
            ),
            (
                "Project Grading",
                root / "env" / "graders.py",
                ["project_docs", "grading", "reward_design"],
            ),
            (
                "Project Tasks",
                root / "env" / "tasks.py",
                ["project_docs", "tasks", "difficulty"],
            ),
            (
                "Project Validation",
                root / "validate.py",
                ["project_docs", "validation", "testing"],
            ),
        ]
        for domain, path, tags in file_specs:
            ...
            for index, section in enumerate(sections, start=1):
                keywords = self._extract_project_keywords(section) or [
                    stem,
                    domain.lower(),
                ]
                chunks.append(
    ...
    def _chunk_project_text(
        self, raw_text: str, chunk_words: int = 140, stride_words: int = 100
    ) -> list[str]:
    ...
    def build_observation(self) -> RagObservation:
        available = [
            ChunkSummary(
                chunk_id=chunk.chunk_id,
                domain=chunk.domain,
    ...
    def _chunk_map(self) -> dict[str, Chunk]:
        if self._chunk_map_cache is None or len(self._chunk_map_cache) != len(
            self.available_chunks
        ):
            self._chunk_map_cache = {
                chunk.chunk_id: chunk for chunk in self.available_chunks
            }
    ...
    def _effective_chunk_text(self, chunk_id: str) -> str:
        ...
        for index, _score, word_count, sentence in sorted(
            ranked_sentences, key=lambda item: (-item[1], item[2], item[0])
        ):
    ...
    def _progress_signals(self) -> dict[str, float]:
        ...
        reviewed_hits = (
            len(required & set(self._reviewed_artifacts)) / len(required)
            if required
            else 1.0
        )
        prioritized_hits = (
            len(required & set(self._selected_chunks)) / len(required)
            if required
            else 1.0
        )
        plan_keywords = sum(
            1
            for keyword in self.task.required_plan_keywords
            if keyword.lower() in self._plan_draft.lower()
        )
        plan_quality = (
            plan_keywords / len(self.task.required_plan_keywords)
            if self.task.required_plan_keywords
            else 1.0
        )
        return {
            ...
            "budget_headroom": round(
                max(0.0, 1.0 - (self._total_tokens_used() / self.task.token_budget)), 3
            ),
        }
    ...
    def _handle_inspect(
        self, chunk_id: str, auto_prioritize: bool
        ...
        reward = (
            0.03
            + (0.08 if self._is_required(chunk_id) else 0.0)
            + (0.05 if is_relevant else 0.0)
        )
        info = {
            "event": "artifact_inspected",
            "artifact_id": chunk_id,
            "hybrid_score": score,
        }
        self._last_action_feedback = "artifact_inspected"
        if auto_prioritize:
            priority_reward, priority_info = self._handle_prioritize(
                chunk_id, inspected=True
            )
    ...
    def _handle_prioritize(
        self, chunk_id: str, inspected: bool = False
    ) -> tuple[float, dict[str, Any]]:
        chunk = self._chunk_map().get(chunk_id)
        if chunk is None:
            self._last_action_feedback = "artifact_not_found"
        ...
        if chunk_id in self._selected_chunks:
            ...
            return 0.0, {
                "event": "artifact_already_prioritized",
                "artifact_id": chunk_id,
            }
        projected_tokens = self._total_tokens_used() + self._effective_chunk_tokens(
            chunk_id
        )
        ...
        domain_bonus = (
            ...
            if len(
                {
                    self._chunk_map()[cid].domain
                    for cid in self._selected_chunks
        ...
        reward = (
            (0.10 if self._is_required(chunk_id) else 0.03)
            + (0.05 if is_relevant else 0.0)
            + domain_bonus
        )
        self._last_action_feedback = "artifact_prioritized"
        return min(reward, 0.18), {
            "event": "artifact_prioritized",
            "artifact_id": chunk_id,
            "hybrid_score": score,
        }
    ...
    def _handle_deprioritize(self, chunk_id: str) -> tuple[float, dict[str, Any]]:
        ...
        return reward, {
            "event": "artifact_deprioritized",
            "artifact_id": chunk_id,
            "required": is_required,
        }

    def _handle_compress(
        self, chunk_id: str, compression_ratio: float
    ) -> tuple[float, dict[str, Any]]:
        chunk = self._chunk_map().get(chunk_id)
        if chunk is None:
            self._last_action_feedback = "artifact_not_found"
        ...
        if self._is_required(chunk_id) and compression_ratio < 0.45:
            ...
            return reward, {
                "event": "overcompressed_required_artifact",
                "artifact_id": chunk_id,
                "hybrid_score": score,
            }
        self._last_action_feedback = "artifact_summarized"
        return reward, {
            "event": "artifact_summarized",
            "artifact_id": chunk_id,
            "hybrid_score": score,
        }
    ...
    def _handle_plan(self, plan: str) -> tuple[float, dict[str, Any]]:
        ...
        coverage = (
            hits / len(self.task.required_plan_keywords)
            if self.task.required_plan_keywords
            else 1.0
        )
